    ...
    with tf.Session() as sess:
        ...
        for i in range(par['n_train_batches_gen']):
            ...
            if i%120 == 0:
                ...
                print('{} | Reconstr. Loss: {:.3f} | Latent Loss: {:.3f} | Task Loss: {:.3f} | Accuracy: {:.3f} | Accuracy2: {:.3f} | <Sig>: {:.3f} +/- {:.3f}'.format( \
                    i, recon_loss, latent_loss, task_loss, acc, acc2, np.mean(sigma), np.std(sigma)))
                if i%120 == 0:
                    iteration.append(i)
                    accuracy.append(acc)
